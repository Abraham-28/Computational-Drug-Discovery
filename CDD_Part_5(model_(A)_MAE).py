# -*- coding: utf-8 -*-
"""CDD_Part_5(model_(A)-MAE).ipynb

# Refer my google colab link for notebook file https://colab.research.google.com/drive/18mW_h0h36hMPHDUg95feIn5Uv0XdLkqQ#scrollTo=73vb7O7BURaq
...
# Loss plot
plt.subplot(1, 2, 1)
plt.plot(train_losses, label='Train Loss (MAE)', color='blue')
...
print(f"Test MSE: {test_mse:.4f} (SOTA: 0.1000), "
      ...
      f"Test MAE: {test_mae:.4f} (SOTA: 0.0500), "
      f"Test R² Score: {test_r2:.4f} (SOTA: 0.9800) ")
...
def evaluate_with_metrics(loader, model, mse_loss_fn, huber_loss_fn):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            all_labels.extend(targets.detach().cpu().numpy().flatten())
...
# First plot: Three loss functions
plt.subplot(1, 2, 1)
metrics = ['MSE', 'Huber Loss', 'MAE']
values = [test_mse, test_huber_loss, test_mae]
colors = ['#1f77b4', '#ff7f0e', '#d62728']  # Distinct colors
...
bars = plt.bar(r2_metric, r2_value, color='#2ca02c')  # Green color
...
plt.ylim([0, 1.0])  # R² typically ranges between 0 and 1
plt.grid(True, axis='y', linestyle='--', alpha=0.3)
...
def evaluate_with_metrics(loader, model, mse_loss_fn, huber_loss_fn):
    ...
    return avg_mse_loss, avg_huber_loss, r2, mae, all_labels, all_preds
...
# Evaluate on the test set
test_mse, test_huber_loss, test_r2, test_mae, all_labels, all_preds = evaluate_with_metrics(test_loader, model, mse_loss_fn, huber_loss_fn)
...
plt.subplot(1, 2, 1)
sns.scatterplot(x=all_labels, y=all_preds, alpha=0.7, color='blue')
plt.plot([min(all_labels), max(all_labels)], [min(all_labels), max(all_labels)],
         'r--', label='Ideal Prediction')
plt.xlabel('Actual pIC50')
plt.ylabel('Predicted pIC50')
plt.title(f'Actual vs Predicted pIC50 Values\n(Test R² Score = {test_r2:.4f}, Test MSE = {test_mse:.4f}, Test Huber Loss = {test_huber_loss:.4f}, Test MAE = {test_mae:.4f})')
plt.legend()
plt.grid(True)
...
#timestamp = eat_time.strftime("MAE_NS3_%Y%m%d_%I%M%S%p")  # %I = 12-hour clock, %p = AM/PM
#timestamp = eat_time.strftime("MAE_Ro5_%Y%m%d_%I%M%S%p")  # %I = 12-hour clock, %p = AM/PM
timestamp = eat_time.strftime("MAE_Clean_%Y%m%d_%I%M%S%p")  # %I = 12-hour clock, %p = AM/PM
